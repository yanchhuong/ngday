...
//	emplInfoPopup
	...
	var top = "20%";
	if ($(window).height() * 8 / 10 < height) { // top 20% 제외한 윈도우 높이가 정해진 높이보다
												// 작으면
		top = "5%";
	}
		...
		if((orgin_date.length == 8) && (Number(orgin_date))){
			if(moment(orgin_date,'YYYYMMDD', true).isValid()){
				if(wehrm.ui.fnCheckYearInRang(curObj, orgin_date)){
					...
					$(curObj).val("");
					parent.jex.warning(name +" 자릿수(8)를 확인하세요.");
		...
		if((orgin_date.length == 8) && (Number(orgin_date))){
			if(moment(orgin_date,'YYYYMMDD', true).isValid()){
				if(wehrm.ui.fnCheckYearInRang(curObj, orgin_date)){
					...
					$(id).val("");
					showPrintInfoMsg("", name +" 자릿수(8)를 확인하세요.", size);
